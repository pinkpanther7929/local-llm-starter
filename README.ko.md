...
fp8 KV cache는 attention backend 후보를 `FLASHINFER`와 `TRITON_ATTN`으로 좁힙니다. FlashInfer kernel은 이 카드를 PCIe bus에서 떨어뜨린 전력이 있어 profile이 `ATTENTION_BACKEND=TRITON_ATTN`을 고정하며, 이 값은 vLLM의 `--attention-backend`로 전달됩니다. 그만큼 attention 처리량은 FlashInfer보다 낮습니다.
